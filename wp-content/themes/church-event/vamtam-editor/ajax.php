...
class WPV_Editor_AJAX {
	...
	private function markup_handler($id, $attributes=null, $content='') {
		...
		?>
			<div id="<?php echo esc_attr($this->get_uniqid($id)) ?>" class="wpv_ed_column wpv_sortable inner-sortable column column-<?php echo preg_replace('/column-(\d)(\d)/', '$1-$2', $id) ?> expandable" data-basename="column">
				<?php echo $this->getControls('size name add edit clone delete handle', preg_replace('/column-(\d)(\d)/', '$1/$2', $id), __('Column', 'church-event'), $attributes) ?>
	...
	private function getControls($controls, $size_str, $name='', $atts = array()) {
		...
		$orig_name = $name;
		if(isset($atts['column_title']) && !empty($atts['column_title']) && $atts['column_title'] !== 'undefined') {
		...
		$controls_html = array(
			...
			'size' => "<div class='column-size-wrapper'>
							<div class='column-increase-decrease'><a class='column-increase icon-plus' href='#' title='".__('Increase width', 'church-event')."'></a> <a class='column-decrease icon-minus' href='#' title='".__('Decrease width', 'church-event')."'></a></div>
							<span class='column-size'>$size_str</span>
						</div>",
			// 'add' => " <a class='column-add icon-plus' href='#' title='".__('Insert element', 'church-event')."'></a> ",
			'clone' => " <a class='column-clone icon-copy' href='#' title='".__('Clone', 'church-event')."'></a> ",
			'edit' => ' <a class="column-edit icon-edit" href="#" title="'.__('Edit shortcode properties', 'church-event').'"></a>',
			'delete' => ' <a class="column-remove icon-remove" href="#" title="'.__('Remove shortcode', 'church-event').'"></a>',
			'handle' => '<div class="handlediv" title="'.__('Click to toggle', 'church-event').'"><br /></div>',
		);
				...
				foreach($sub_contents as $i=>$text): ?>
					...
					<div>
						<h3 class="title-wrapper clearfix">
							...
							<a class="accordion-remove icon-remove" title="<?php _e('Remove', 'church-event') ?>"></a>
							<a class="accordion-clone icon-copy" title="<?php _e('Clone', 'church-event') ?>"></a>
							<a class="accordion-background-selector" data-background-image="<?php echo esc_attr($params[$i]['background_image']) ?>" title="<?php esc_attr_e('Change Pane Background', 'church-event') ?>"><?php wpv_icon('image') ?></a>
						</h3>
			...
			case 'tabs':
				...
				?>
					<li>
						...
						<a class="wpv-icon-selector-trigger tab-icon-selector <?php echo wpv_get_icon_type($p['icon']) ?> <?php if(empty($p['icon'])) echo 'no-icon' ?>" data-icon-name="<?php echo $p['icon'] ?>" title="<?php esc_attr_e('Change Icon', 'church-event') ?>"><?php wpv_icon($p['icon']) ?></a>
					</li>
				<?php endforeach;
